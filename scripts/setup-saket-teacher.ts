...
async function main() {
  const email = 'saket7174@gmail.com';
  const name = 'Saket Verma';
  const tempPassword = 'TT-FAC-Saket#2026';
  const hashedPassword = await bcrypt.hash(tempPassword, 12);
  const facultyId = 'TT-FAC-0003';

  console.log('=== Step 1: Handling potential Faculty ID conflict ===');
  // Check if there is another teacher using TT-FAC-0003
  ...
  if (conflictingTeacher) {
    ...
    // Shift the conflicting teacher's ID to TT-FAC-0004
    await prisma.teacher.update({
      where: { id: conflictingTeacher.id },
      data: { teacherId: 'TT-FAC-0004' },
    });
    console.log(`✓ Updated conflicting teacher's Faculty ID to TT-FAC-0004`);
  }

  console.log('\n=== Step 2: Creating or Updating Saket Verma User Account ===');
  ...
  console.log('\n=== Step 3: Upserting Teacher Profile for Saket Verma ===');
