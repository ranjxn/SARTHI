    ...
    >
      <AnimatePresence initial={false} mode="sync">
        {isComplete ? (
          ...
          >
            <CheckCircleIcon size={18} className="fill-emerald-500 text-white" />
          </motion.span>
        ) : (
          <motion.span
            animate={{ opacity: 1, transform: "scale(1)" }}
...
function ReceiptPrinterOutput({
  ...
  return (
    <div
      className={cn(
        "relative z-50 -mt-4 h-[24rem] xs:h-[26rem] sm:h-[32rem] w-[calc(80%+3rem)] max-w-full px-6",
        stage === "complete" ? "overflow-y-auto" : "overflow-hidden",
        className,
      )}
      {...props}
    >
