...
export const StudentStats = ({ summary, isLoading }: StudentStatsProps) => {
  const stats = [
    {
      title: 'Total Students',
      value: isLoading ? '...' : summary?.totalStudents || 0,
      label: 'Lifetime Learners',
      icon: Users,
      color: 'blue'
    },
    {
      title: 'Active Now',
      value: isLoading ? '...' : summary?.activeThisWeek || 0,
    ...
    {
      title: 'New Onboard',
      value: isLoading ? '...' : summary?.newThisMonth || 0,
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
      {stats.map((stat, i) => (
        <div key={i} className="relative overflow-hidden bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] group hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] hover:-translate-y-[6px] transition-all duration-500">
